...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        rows = "".join(
            ...
            for k, v in last_check_results.items()
        )
        html = f"""<html><body style="font-family:sans-serif;padding:20px">
        <h2>🚗 Hot Wheels Checker — Running ✅</h2>
        <p><b>Last check:</b> {last_check_time}</p>
        <p><b>Interval:</b> every {CHECK_INTERVAL_MINUTES} minutes</p>
        <p><b>Alerting:</b> {YOUR_WHATSAPP_NUMBER}</p>
        <hr/>{rows}</body></html>""".encode()
